

ZArchive : File {

	var stringCache, <>path, <>version=1.0;
	var lastItem, count=0, stringCacheStart;

	*write { arg pathName;
		^super.new(pathName, "wb").path_(pathName).check.initStringCache
	}
	*read { arg pathName;
		^super.new(pathName, "rb").path_(pathName).check
			.getStringCache
	}
	writeItem { arg thing, extraArgs;
		// == has some problems
		if(lastItem === thing, {
			count = count + 1;
		}, {
			lastItem = thing;
			this.saveCount;
			thing.writeZArchive(this, extraArgs);
		});
	}
	readItem { arg assertClass;
	    // if assertClass not nil, it will assert that the item is of that class

		var type, size, thing, classname;
		if(count > 0, {
			count = count - 1;
			^lastItem
		});
		type = this.getChar;

		if(type == $., { // ... repetition
			count = this.getInt32 - 1;
			^lastItem
		});

		if(type == $N, {
			if(assertClass.notNil and: (Nil !== assertClass), {
				Error("ZArchive got wrong type:"+nil+"expected:"+assertClass.asString).throw;
			});
			^lastItem = nil
		});
		if(type == $F, {
			thing = this.getFloat;
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			^lastItem = thing
		});
		if(type == $I, {
			thing = this.getInt32;
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			^lastItem = thing
		});
		if(type == $s, { // small string < 128
			thing = this.readString;
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			^lastItem = thing
		});
		if(type == $S, { // large string
			thing = this.readLargeString;
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			^lastItem = thing
		});
		if(type == $y, { // symbol
			thing = this.readString.asSymbol;
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			^lastItem = thing
		});
		if(type == $D, {// dictionaries
			classname = this.readString;
			size = this.getInt32;
			thing = classname.asSymbol.asClass.new(size);
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			size.do({ arg i;
				thing.put(this.readItem, this.readItem);
			});
			^lastItem = thing
		});
		if(type == $C, { // collection
			classname = this.readString;
			size = this.getInt32;
			thing = classname.asSymbol.asClass.new(size);
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			size.do({ arg i;
				thing.add( this.readItem );
			});
			^lastItem = thing
		});
		if(type == $x, { // small compile string
			thing = this.readString.interpret;
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			^lastItem = thing
		});
		if(type == $X, { // large compile string
			thing = this.readLargeString.interpret;
			if(assertClass.notNil and: (thing.class !== assertClass), {
				Error("ZArchive got wrong type:"+thing+"expected:"+assertClass.asString).throw;
			});
			^lastItem = thing
		});
		if(this.pos >= (stringCacheStart), {
			this.close;
			Error("End of file, unable to read item"+this).throw
		});
		this.close;
		Error("ZArchive read error, got bad type code:"+type).throw;
	}

	writeClose {
		var stringCacheStart;
		this.saveCount;
		stringCacheStart = this.pos;

		// if you do go over the limit, you could simply raise the limit but your old files aren't compatible
		if(stringCache.size >= (2**16), {
			Error("Too many strings in archive !  Not supported.").throw;
		});

		this.putInt16(stringCache.size);
		stringCache.keysValuesDo({ |string, index|
			this.putInt16(index);
			this.putInt8(string.size);
			this.putString(string);
		});
		this.putInt32(stringCacheStart);

		this.close;
	}

	//PRIVATE
	saveCount {
		if(count > 0, {
			super.putChar($.);
			super.putInt32(count);
			count = 0;
		});
	}
	check {
		if(this.isOpen.not, {
			if(File.exists(path).not, {
				Error("ZArchive: no such file" + path).throw;
			}, {
				Error("ZArchive failed to open" + path).throw;
			});
		});
	}
	initStringCache { stringCache = Dictionary.new; lastItem = this; }
	getStringCache {
		var size;
		this.seek(-4, 2); // bytes from the end
		stringCacheStart = this.getInt32;

		this.seek(stringCacheStart, 0);
		size = this.getInt16;
		stringCache = Array.newClear(size);
		size.do({ arg i;
			var index, ssize, string;
			index = this.getInt16;
			ssize = this.getInt8;
			string = String.newClear(ssize);
			this.read(string);
			stringCache.put( index, string );
		});
		this.seek(0, 0);

		// the one thing you will never archive is the archiver itself
		// so lastItem starts with this
		lastItem = this;
		// any other item in existence is something you might potentially save next
		// eg. Object, nil
	}

	// don't use these or the File superclass methods unless you know what you are doing.
	// you must use saveCount correctly or you will corrupt data when there are repeat
	// values.  Stick to the public writeItem/readItem, make music and be happy.
	writeString { arg string;
		var prev;
		prev = stringCache[string];
		if(prev.isNil, {
			prev = stringCache.size;
			stringCache[string] = prev;
		});
		this.putInt16(prev);
	}
	readString {
		var ix, string;
		ix = this.getInt16;
		^stringCache[ix] ?? {Error("ZArchive failed to find String : index:"+ix).throw};
	}
	writeLargeString { arg string;
		this.putInt32(string.size);
		this.putString(string);
	}
	readLargeString {
		var size, string;
		size = this.getInt32;
		string = String.newClear(size);
		this.read(string);
		^string
	}

	rescindItem { arg item;
		if(item == lastItem, {
			count = count + 1;
		});
	}
	asZArchive { ^this }
}
